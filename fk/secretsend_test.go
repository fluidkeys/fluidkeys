...
const (
	englishAlphabet     = "AaBbCcDdEeFfGgHhIiJjKkLlMmNnOoPpQqRrSsTtUuVvWwXxYyZz"
	printableCharacters = "☠☮☯♠Ω♤♣♧♥♡♦♢♔♕♚♛⚜★☆✮✯☄☾☽☼☀☁☂☃☻☺☹۞۩ εїзƸ̵̡Ӝ̵̨̄ƷξЖЗεжз☎☏¢☚☛"
	sampleEmojis        = "😀😁😂🤣😃😄😅😆😉😊😋😎😍😘😗😙😚🙂"
	breaks              = "line 1\rline 2\n"
)
...
func TestContainsDisallowedRunes(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(fmt.Sprintf("for '%v')", test.inputString), func(t *testing.T) {
			assert.Equal(t, test.expected, containsDisallowedRune(test.inputString))
		})
	}
	...
	for name, code := range invalidColorCharacters {
		t.Run(fmt.Sprintf("for color '%v')", name), func(t *testing.T) {
	...
	for name, code := range invalidANSICodes {
		t.Run(fmt.Sprintf("for '%v')", name), func(t *testing.T) {
			if containsDisallowedRune(code) != true {
	...
	validANSICodes := map[string]string{
		"LF": "\012", // Line Feed
		"CR": "\015", // Carriage Return
	}

	for name, code := range validANSICodes {
		t.Run(fmt.Sprintf("for '%v')", name), func(t *testing.T) {
			if containsDisallowedRune(code) != false {
				t.Fatalf("expected '%v' (%v) to be allowed but it wasn't", code, name)
			}
		})
	}

	for _, naughtyString := range naughtystrings.Unencoded() {
		t.Run(fmt.Sprintf("'%v' doesn't crash it", naughtyString), func(t *testing.T) {
