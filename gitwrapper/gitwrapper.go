...
// Load finds the local git binary, ensures it has a valid-looking version string and
// returns a *Git pointer, or error if git can't be found.
func Load() (*Git, error) {
	git := Git{}

	version, err := git.Version()
	if err != nil {
		return nil, fmt.Errorf("error loading git: %v", err)
	}

	log.Printf("found git binary with version %s", version)
	return &git, nil
}
...
// run runs the given command and returns stdout, stderr and any error encountered
func (g *Git) run(arguments ...string) (
